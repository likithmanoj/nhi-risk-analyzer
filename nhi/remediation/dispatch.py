...
def dispatch_remediation(findings: list, dry_run: bool = True) -> dict:
    ...
    for finding in findings:
        ...
        if ignored:
            ...
            continue
        try:
            handler = HANDLER_MAP.get(finding.get('RuleID'))
            if handler:
                ...
                logger.error(f'No handler registered for this rule: {finding.get('RuleID')}')
                failed+=1
        except Exception as e:
            logger.exception(f"Failed to remediate finding for RuleID '{finding.get('RuleID')}': {e}")
            failed += 1      
    return{
        "total": len(findings),
